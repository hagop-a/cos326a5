...
sig
  ...
  let union s1 s2 = D.fold (fun k _ acc -> insert k acc) s1 s2

  let intersect s1 s2 =
    D.fold (fun k _ acc -> if D.member s2 k then insert k acc else acc) empty s1
...
struct
  ...
  let test_remove () =
    ...
    let test_union () =
      let elts1 = List.init 50 (fun _ -> C.gen_random ()) in
      let elts2 = List.init 50 (fun _ -> C.gen_random ()) in
      let s1 = insert_list empty elts1 in
      let s2 = insert_list empty elts2 in
      let s_union = union s1 s2 in
      List.iter (fun x -> assert (member s_union x)) (elts1 @ elts2);
      ()
    

      let test_intersect () =
        let common_elts = List.init 30 (fun _ -> C.gen_random ()) in
        let elts1 = common_elts @ List.init 20 (fun _ -> C.gen_random ()) in
        let elts2 = common_elts @ List.init 20 (fun _ -> C.gen_random ()) in
        let s1 = insert_list empty elts1 in
        let s2 = insert_list empty elts2 in
        let s_intersect = intersect s1 s2 in
        List.iter (fun x -> assert (member s_intersect x)) common_elts;
        List.iter (fun x -> assert (not (member s_intersect x))) (List.filter (fun x -> not (List.mem x common_elts)) (elts1 @ elts2));
        ()
...
module DictSet(C : COMPARABLE) : (SET with type elt = C.t) = 
  ...
  let fold f acc s = D.fold (fun k _ acc' -> f k acc') acc s
